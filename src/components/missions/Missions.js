...
const Missions = props => {
  ...
  const handleMissionClick = missionName => {
    ...
    if (props.userStats.stamina.value < mission.stamCost) {
      console.log(
        `not enough stamina, stamina: ${props.userStats.stamina.value}`
      );
      return;
    }
    ...
    <div>
      <div className=''>
        <div className=''>
          <Tooltip
            ...
            title='Number inside brackets is stamina cost'
          >
            <b>Missions</b>
          </Tooltip>
        </div>
      ...
      {Object.entries(props.missions).map(mission => {
        return (
          <div key={mission[0]}>
            <div className=''>
              <Tooltip title={`MISSION INFO HERE ${mission[1].description}`}>
                {
